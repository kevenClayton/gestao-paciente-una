...
$(function(){
    ...
    var floatMaskBehavior = function (val) {
        return val.replace(/\D/g, '').length === 3 ? '00.00' : '009.99';
    };

    floatOptions = {
        onKeyPress: function(val, e, field, options) {
            field.mask(floatMaskBehavior.apply({}, arguments), options);
        }
    };
    ...
    $(".mask-float").mask(floatMaskBehavior, floatOptions);
    // $(".mask-float").mask('00.00', {reverse: true, clearIfNotMatch: true});
    $(".mask-temp").mask('00.00 ºC', {reverse: true, clearIfNotMatch: true});
